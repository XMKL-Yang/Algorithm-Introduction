/*
 *   STUDENT NAME      :  Yujing Yang 
 *   STUDENT ID        :  260827923
 *   
...
public class MyBigInteger  implements Comparable<MyBigInteger> {   //  We will cover what "implements Comparable" means later
	...
	public MyBigInteger dividedBy (MyBigInteger divisor) throws Exception{
		...
		MyBigInteger remainder = new MyBigInteger(0,this.base);
		MyBigInteger ZERO = new MyBigInteger(0,this.base);
		int num = this.coefficients.size()-1;
		
		if (this.base != divisor.base){
			//System.out.println("ERROR: bases must be the same in a division");
			throw new Exception("Bases must be the same in a division!");
		}

		if (divisor.coefficients.size()==1 && divisor.coefficients.get(0) == 0) {
			//System.out.println("ERROR: divisor cannot equal to 0 ");
			throw new Exception("ERROR: divisor cannot equal to 0!");
		}

		while(num >= 0 ){			
			quotient = quotient.timesBaseToThePower(1);
			if (remainder.compareTo(ZERO) == 1) remainder = remainder.timesBaseToThePower(1);
			remainder.coefficients.set(0,this.coefficients.get(num));
			
			while (remainder.compareTo(divisor) >= 0){
				remainder = remainder.minus(divisor);
				Integer temp = quotient.coefficients.get(0);
				temp += 1;
				quotient.coefficients.set(0,temp);
			}			
			num--;
		}		
		return quotient;		
	...
	public MyBigInteger convert(int newBase) throws Exception {
		...
		if (newBase < 2 || newBase > 10){
			//System.out.println("New base should with the range from 2 to 10");
			throw new Exception("New base should with the range from 2 to 10");
		}

		MyBigInteger NEWBASE = new MyBigInteger(newBase, base);
		MyBigInteger ZERO = new MyBigInteger(base);
		ZERO.coefficients.add(0);
		//MyBigInteger OLDBASE = new MyBigInteger(base, base);
		MyBigInteger BASETEN = new MyBigInteger(base,10);

		MyBigInteger remainder;
		MyBigInteger result = new MyBigInteger(newBase);
		result.coefficients.add(0);
		MyBigInteger quotient = this.clone();
		...
		if (newBase==base){
			...
			int j = 0;
			while (quotient.compareTo(ZERO) > 0) {
				remainder = quotient.mod(NEWBASE);
				quotient = quotient.dividedBy(NEWBASE);
				MyBigInteger rem = new MyBigInteger(remainder.coefficients.get(0), newBase);
				result = result.plus(rem.timesBaseToThePower(j));
				j++;
			}
			return result;
		}		
		 else {
			// what to do if newBase is larger than oldBase
			MyBigInteger num = new MyBigInteger(0,10);
			for (int k = this.coefficients.size()-1; k >= 0 ; k--){
				MyBigInteger temp = new MyBigInteger(this.coefficients.get(k), 10);
				for (int i = k; i > 0; i--) {
					temp = temp.times(BASETEN);
				}
				num = num.plus(temp);
			}
			 result = num.convert(newBase);
		}		
		return result;
	}
	...
	public ArrayList<MyBigInteger>  primeFactors() throws Exception {
		...
		MyBigInteger num = new MyBigInteger(3, base);	
		//num.coefficients.add(0);
		//num.coefficients.set(0, 3);
		
		MyBigInteger dividend = this.clone();
		...
		while (dividend.mod(TWO).compareTo(ZERO) == 0){
			dividend = dividend.dividedBy(TWO).clone();
			factors.add(TWO);
		}
		
//		while (dividend.mod(THREE).compareTo(ZERO) == 0){
//			dividend = dividend.dividedBy(THREE).clone();
//			factors.add(THREE);
//		}
//		
		MyBigInteger squareInt = dividend.square();	
		
		//int time = 5;
		while (num.compareTo(squareInt)<=0) {	
			while (dividend.mod(num).compareTo(ZERO) == 0){
				dividend = dividend.dividedBy(num);
				factors.add(num);
				squareInt = dividend.square();
			}	
...
//			}
		...
		if (!(dividend.mod(num).coefficients.size()==1 && dividend.mod(num).coefficients.get(0)==1)) {
	...
	private MyBigInteger square() throws Exception {
		  
		MyBigInteger result = new MyBigInteger (1,base);	
		MyBigInteger a = this.clone();
		MyBigInteger two = new MyBigInteger (2,base);	
		//MyBigInteger zero = new MyBigInteger (0,base);
		
		while(result.times(result).compareTo(a)<0) {
...
//		if (size/2.00 - size/2 ==0) {
//			// this is even number
//			for (int i = 0; i < this.coefficients.size()/2; i++) {
//				result.coefficients.add(j, this.coefficients.get(i));
//				j++;
//			}
//		}else {
//			// this is odd number
//			for (int i = 0; i < this.coefficients.size()/2+1; i++) {
//				result.coefficients.add(j, this.coefficients.get(i));
//				j++;
////			}
////		}
